...
++  expects
  ...
  ^-  test-data-cplex
  =.  e  [~ ~]
  :: =/  cp  (~(put bi:mip *(mip:mip @ @ @)) 1 2 3)
  =/  c  ~[(malt (limo ~[li+2 ad+'Best' su+'Books' pe+'All-time']))]
  :: =/  cp  (~(put bi:mip *(mip:mip @u @u (list cate))) 1 2 c)
  =/  cp  (~(put bi:mip *(mip:mip @u @u (list cate))) 1 2 ~)
  =:  c0.e   [~ [me=[id=[sh=~zod uu=~.jbl03.q1tnj] lf=0 rf=0 ts=[cr=~2024.4.1..20.31.25..2be3 de=~]] da=cp]]
...
++  test-category-plex-creation
  ...
  %+  expect-eq
    ...
    !>
      ...
      =/  d  (setup)
      =/  c  (need c0.d)
      (~(new category-plex fake-bowl-0) c)
  ==
::
--
